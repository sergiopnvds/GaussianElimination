...
void gauss() {
  ...
  //printf("********* value of roww size %lu\n", sizeof(row0Buffer)/sizeof(float));
  //printf("********* value of buffer size %lu\n", sizeof(valueBuffer)/sizeof(float));
...
void addingProcess(){
  ...
  while( go == 1){
    ...
    MPI_Recv(valueBufferpProcess, 1, MPI_FLOAT, 0 , 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    ...
    for (j=0; j <nRowsPerThread; j++){
      multiplier = valueBufferpProcess[j+j+norm] / row0BufferProcess[norm];
